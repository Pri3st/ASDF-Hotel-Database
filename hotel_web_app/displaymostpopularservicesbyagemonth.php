...
<html>
<body bgcolor="1c1d26">
<a href="mostpopularservicesbyagemonth.php" class="button">Back</a>
        <head>
                <link rel="stylesheet" type="text/css" href="button.css">
        </head>
<center><h2 style="color:white;">Most Popular Services by Selected Age Category This Month</h2></center>
<center><h4 style="color:white;">"Number of Distinct Users" Column shows all the distinct Customers that have made use of that specific Hotel Service.</h4></center>
<center><h4 style="color:white;">Multiple uses of a Hotel Service by the same Customer do not count to the query result.</h4></center>
<br>
...
$query0 = "SELECT Service.service_id, COUNT(DISTINCT Receive.nfc_id) AS Freq, Service.service_descr FROM Receive, Service, CustomerView WHERE CAST(Receive.charge_time as DATE)>'2021-05-31' AND Receive.nfc_id=CustomerView.nfc_id AND CustomerView.Age<41 AND Receive.service_id=Service.service_id AND Receive.service_id!='Rm2' AND Receive.service_id!='Rm4' GROUP BY Receive.service_id ORDER BY Freq DESC";
...
$query1 = "SELECT Service.service_id, COUNT(DISTINCT Receive.nfc_id) AS Freq, Service.service_descr FROM Receive, Service, CustomerView WHERE CAST(Receive.charge_time as DATE)>'2021-05-31' AND Receive.nfc_id=CustomerView.nfc_id AND CustomerView.Age>40 AND CustomerView.Age<61 AND Receive.service_id=Service.service_id AND Receive.service_id!='Rm2' AND Receive.service_id!='Rm4' GROUP BY Receive.service_id ORDER BY Freq DESC";
...
$query2 = "SELECT Service.service_id, COUNT(DISTINCT Receive.nfc_id) AS Freq, Service.service_descr FROM Receive, Service, CustomerView WHERE CAST(Receive.charge_time as DATE)>'2021-05-31' AND Receive.nfc_id=CustomerView.nfc_id AND CustomerView.Age>60 AND Receive.service_id=Service.service_id AND Receive.service_id!='Rm2' AND Receive.service_id!='Rm4' GROUP BY Receive.service_id ORDER BY Freq DESC";
